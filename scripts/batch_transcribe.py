"""Batch transcribe all TCM audio files (CPU, small model)."""
import json, time
...
wavs = sorted(audio_dir.glob('*.wav'), key=lambda f: f.stat().st_size)
...
for f in wavs:
    ...
    try:
        ...
        print(f"  OK: {elapsed:.0f}s (x{ratio:.1f} realtime) | {info.duration:.0f}s audio | {len(texts)} segments")
...
print(f"\nDone: {done}/{len(wavs)} files in {total_elapsed/60:.1f} minutes")
